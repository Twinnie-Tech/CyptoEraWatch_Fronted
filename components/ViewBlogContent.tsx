...
interface ViewBlogContentProps {
    content:string;
}

const ViewBlogContent = ({content}: ViewBlogContentProps) => {
    return (
        <AlertDialog.Root>
            <AlertDialog.Trigger>
            <TbListDetails  data-tooltip-id="tooltip"   className="text-blue-600 text-2xl" />
            <Tooltip id="tooltip" place="top"  content="View Article"/> 
            </AlertDialog.Trigger>
            <AlertDialog.Portal>
                ...
                <AlertDialog.Content className="data-[state=open]:animate-contentShow fixed top-[50%] left-[50%] h-[auto] w-[90vw] max-w-[600px] translate-x-[-50%] translate-y-[-50%] rounded-[6px] bg-white p-[25px] shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px] focus:outline-none">
                    ...
                    <div className="absolute bottom-0 right-1 flex justify-end gap-[25px] w-full mt-5 p-3">
                        <AlertDialog.Action asChild>
                            <button className="text-red11 bg-red-600 hover:bg-red-400 focus:shadow-red7 inline-flex h-[35px] items-center justify-center rounded-[4px] px-[15px] font-medium leading-none outline-none focus:shadow-[0_0_0_2px]">
                                Cancel
                            </button>
                        </AlertDialog.Action>
                    </div>
